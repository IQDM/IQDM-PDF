...
class Delta4Report:
    def __init__(self):
        ...
        self.treatment_summary_columns = [
            "Beam",
            "Gantry",
            "Energy",
            "Daily Corr",
    ...
    def __call__(self, file_path):
        ...
        # Beam
        self.index_start["Beam"] = self.get_index_of_next_text_block(
            self.get_string_index_in_text("°")
        )
        self.index_end["Beam"] = (
            self.get_index_of_next_text_block(self.index_start["Beam"]) - 1
        )
        ...
        # Gantry
        self.index_start["Gantry"] = self.get_index_of_next_text_block(
            self.index_end["Beam"]
        )
        self.index_end["Gantry"] = (
            self.get_index_of_next_text_block(self.index_start["Gantry"]) - 1
        )
        self.data["Gantry"] = ["N/A"] + self.get_data_block("Gantry")
        for composite_name_option in COMPOSITE_BEAM_NAMES:
            if composite_name_option in self.data["Gantry"]:
                self.data["Gantry"].pop(
                    self.data["Gantry"].index(composite_name_option)
                )

        energy_override = (
            []
        )  # sometimes the energy is on the same line as the gantry
        for i, row in enumerate(self.data["Gantry"]):
            self.data["Gantry"][i] = row.replace("\xc2", "").replace(
                "\xb0", ""
            )
            ...
            if len(row_split) > 3:
                ...
                self.data["Gantry"][i] = (
                    self.data["Gantry"][i]
                    .replace(energy_override[-1], "")
                    .strip()
                )

        # Dose and analysis
        self.index_start["Analysis"] = (
            self.get_string_index_in_text("Daily corr Norm") + 2
        )
        self.index_end["Analysis"] = (
            self.get_index_of_next_text_block(self.index_start["Analysis"]) - 1
        )
        analysis_data_block = self.text[
            self.index_start["Analysis"] : self.index_end["Analysis"]
        ]
        analysis_data = []
        while analysis_data_block:
            ...
            if "Gy" in row and "%" in row:
                ...
                split = ["Gy", "cGy"][
                    "cGy" in row[0]
                ]  # Report may be in cGy or Gy
                data = [
                    row[0].split(split)[0].strip(),
        ...
        if "factor" in self.data["Dev"][0]:
            self.data["Dev"][0] = (
                self.data["Dev"][0].replace("factor", "").strip()
            )

        # Daily Correction Factor
        self.index_start["Daily Corr"] = self.get_index_of_next_text_block(
            self.index_end["Analysis"]
        )
        if (
            "Det within acceptance"
            in self.text[self.index_start["Daily Corr"]]
        ):
            self.index_start["Daily Corr"] = self.get_index_of_next_text_block(
                self.index_start["Daily Corr"]
            )
            if "index dose dev" in self.text[self.index_start["Daily Corr"]]:
                self.index_start[
                    "Daily Corr"
                ] = self.get_index_of_next_text_block(
                    self.index_start["Daily Corr"]
                )
        ...
        self.index_end["Daily Corr"] = (
            self.get_index_of_next_text_block(self.index_start["Daily Corr"])
            - 1
        )
        ...
        if self.data["Energy"] is None:
            self.index_start["Energy"] = self.get_index_of_next_text_block(
                self.index_end["Daily Corr"]
            )
            if "dose dev" in self.text[self.index_start["Energy"]]:
                self.index_start["Energy"] = self.get_index_of_next_text_block(
                    self.index_start["Energy"]
                )
            self.index_end["Energy"] = (
                self.get_index_of_next_text_block(self.index_start["Energy"])
                - 1
            )
            self.data["Energy"] = ["N/A"] + self.get_data_block("Energy")
            if any(
                energy_override
            ):  # replace values with overrides found in Gantry code block
        ...
        # Gamma Criteria
        self.index_start["Gamma Criteria"] = self.text.index(
            "Parameter Definitions & Acceptance Criteria, Detectors"
        )
        self.index_start["Acceptance Limits"] = self.text.index(
            "Acceptance Limits"
        )
        self.index_end["Gamma Criteria"] = (
            self.index_start["Acceptance Limits"] - 1
        )
        self.index_end["Acceptance Limits"] = (
            self.get_index_of_next_text_block(
                self.index_start["Acceptance Limits"]
            )
            - 1
        )
        ...
        self.data["gamma_pass"] = self.get_data_block("Acceptance Limits")[
            -1
        ].split("%")[0]
    ...
    def get_index_of_first_date(self):
        for i, row in enumerate(self.text):
            if are_all_strings_in_text(
                row, ["/", ":", "M"]
            ) or are_all_strings_in_text(row, [".", ":", "M"]):
                try:
    ...
    def get_data_block(self, data_type):
        return self.text[
            self.index_start[data_type] : self.index_end[data_type]
        ]
    ...
    def summary_data(self):
        try:
            daily_corr = sum(
                [float(f) for f in self.data["Daily Corr"] if f != "N/A"]
            ) / (len(self.data["Daily Corr"]) - 1)
        except Exception as e:
            print(e)
            print(
                "WARNING: Could not process daily corr for %s - %s"
                % (self.data["patient_name"], self.data["patient_id"])
            )
            daily_corr = 1.0

        return {
            "Patient Name": self.data["patient_name"],
            "Patient ID": self.data["patient_id"],
            "Plan Date": self.measured_date,
            "Energy": "/".join(
                list(set([e for e in self.data["Energy"] if e != "N/A"]))
            ),

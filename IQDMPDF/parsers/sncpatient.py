...
class SNCPatientReport:
    ...
    def process_data(self, text_data):
        ...
        self.data["qa_file_parameter"] = self.get_group_results(
            "QA File Parameter"
        )
        ...
        self.data["dose_comparison"] = self.get_group_results(
            self.data["dose_comparison_type"]
        )
        if "% Diff" in list(
            self.data["dose_comparison"]
        ):  # Alternate for Difference (%) for some versions of report?
            self.data["dose_comparison"]["Difference (%)"] = self.data[
                "dose_comparison"
            ]["% Diff"]
        if "Threshold" in list(
            self.data["dose_comparison"]
        ):  # Alternate for Threshold (%) for some versions of report?
            self.data["dose_comparison"]["Threshold (%)"] = self.data[
                "dose_comparison"
            ]["Threshold"]
        ...
        self.data["summary"] = self.get_group_results(
            "Summary (%s Analysis)" % self.data["analysis_type"]
        )

        # Gamma Index Summary Block
        try:
            self.text.index("Gamma Index Summary")
            self.data["gamma_stats"] = self.get_gamma_statistics(
                "Gamma Index Summary"
            )
        except ValueError:
            self.data["gamma_stats"] = {
                "Minimum": "n/a",
                "Maximum": "n/a",
                "Average": "n/a",
                "Stdv": "n/a",
    ...
    def get_group_results(self, data_group):
        ...
        data_start = (
            self.text[var_name_start:-1].index("") + 1 + var_name_start
        )
        ...
        for i in range(data_count):
            if self.text[var_name_start + i]:
                group_results[self.text[var_name_start + i]] = self.text[
                    data_start + i
                ].replace(" : ", "")
    ...
    def summary_data(self):
        ...
        """
        patient_name = (
            self.data["qa_file_parameter"]["Patient Name"]
            .replace("^", " ")
            .split(", ")
        )
        ...
        return {
            ...
            "Meas Uncertainty": self.data["dose_comparison"][
                "Meas Uncertainty"
            ],
